...
#     from_checkpoint=$task.ddpo.61/$time_stamp/val_best.ckpt \
...
CUDA_VISIBLE_DEVICES=1 PYTHONPATH=. python diffusion/eval.py method=eval_guided task=ibm.cluster512.v1 \
    from_checkpoint=$task.ddpo.61/$time_stamp/val_best.ckpt num_output_samples=18 time_stamp=$time_stamp

# 测试train
CUDA_VISIBLE_DEVICES=1 PYTHONPATH=. python diffusion/eval.py method=eval_guided task=ibm.cluster512.v1 \
    from_checkpoint=$task.ddpo.61/$time_stamp/train_best.ckpt num_output_samples=18 time_stamp=${time_stamp}_train_best
...
#     method=eval from_checkpoint=$task.ddpo.61/$time_stamp/val_best.ckpt \
...
#         task=ibm.cluster512.v1 from_checkpoint=$task.ddpo.61/$time_stamp/val_best.ckpt \
