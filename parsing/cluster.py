...
def cluster_lefdef(lef_path, def_path, out_dir, pickle_out_dir=None, pickle_idx=None, num_clusters=512, cluster_ubfactor=5, ref_placement_path = None, verbose=False, mode="remove_first", cluster_algorithm="hmetis"):
    ...
    is_skip = (num_clusters == 0) or (os.path.exists(def_out_path) and os.path.exists(lef_out_path)) # is_skip = True
    ...
    if ref_placement_path is not None: # ref_placement_path = None
    ...
    scale_factor = parse_utils.DesignComponent.mult # I hate this global variable nonsense  # 100
    ...
    if pickle_out_dir is not None:
        ...
        utils.write_to_pickle(clustered_placement.squeeze(dim=0), clustered_graph, pickle_out_dir, pickle_idx)  # 保存在 datasets/graph/ibm.cluster512.v1
        # clustered_graph = cond
    metrics = {
        "name": re.search("(.*).lef$",lef_path.name).group(1),
        "num_vertices": clustered_graph.x.shape[0],
        "num_edges": clustered_graph.edge_index.shape[1],
...
def main(cfg):
    ...
    for i, def_path in enumerate(def_files):
        circuit_dir = def_path.parent # PosixPath('benchmarks/ibm/ibm01')
        def_name = def_path.name  # 'ibm01.def'
        ...
        if cfg.placement_dir is not None:  # 这里是none
        ...
        circuit_out_dir = os.path.join(out_dir, circuit_dir.name)  # 'benchmarks/ibm.cluster512.v1/ibm01'
        ...
        circuit_metrics = cluster_lefdef(
            lef_path,  # benchmarks/ibm/ibm01/ibm01.lef
            def_path,  # benchmarks/ibm/ibm01/ibm01.def
            circuit_out_dir,  # 'benchmarks/ibm.cluster512.v1/ibm01'
            pickle_out_dir=pickle_out_dir,  # 'datasets/graph/ibm.cluster512.v1'
            pickle_idx=pickle_idx,  # 0
            num_clusters=num_clusters, # 512
            cluster_ubfactor=cfg.ubfactor, # 5
            verbose=cfg.verbose, # False
            mode=cfg.mode, # remove_first
            cluster_algorithm=cfg.cluster_algorithm, # hmetis
            ref_placement_path=placement_path, # None
            )
