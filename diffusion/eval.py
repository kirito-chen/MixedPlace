...
def main(cfg):
    ...
    if cfg.legalization.mode in [None, "none", "None", ""]:  
        legalize_fn = None
    elif cfg.legalization.mode == "scheduled":  # ibm_guide 会进入这
    ...
    if cfg.logger.get("wandb", False):
        wandb_run_name = f"{cfg.task}.{cfg.method}.{cfg.seed}.{cfg.time_stamp}" if not cfg.param_sweep else None
    ...
    if load_state is False:
        print(f"checkpointer.load: Fail to load {cfg.from_checkpoint}")
        exit(11)
    ...
    for i in range(cfg.num_output_samples):
        ...
        metrics, metrics_special, image, image_legalized = utils.save_outputs(
            ...
            output_number_offset=i,  # i | 0  ！！！！！！！！！！！记得修改这里
            policy=cfg.eval_policy_algorithm,
        ...
        # === 清理GPU显存 ===
        # del x, cond, image, image_legalized, metrics, metrics_special, eig_vals
        # torch.cuda.empty_cache()  # 释放空闲显存
        # torch.cuda.synchronize()  # 同步等待显存释放
    ...
    logger.write()  # 这里输出的Step 0的信息，是前面的平均情况
