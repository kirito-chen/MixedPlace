...
from omegaconf import open_dict # 用于处理 Hydre config
...
class AttGNNBlock_slide_window(nn.Module):
    def __init__(
        self,
        in_node_features,
        out_node_features,
        ...
        **kwargs
        ...
        for i in range(num_layers):
            ...
            # extra embed for attention input
            self._att_extra_input_embed_layers.append(
                nn.Linear(cond_node_features + attention_extra_features, hidden_node_features)
            )

            # sliding window multihead attention layer
            self._attention_layers.append(
                SlidingWindowMultiHeadAttention(
                    embed_dim=hidden_node_features,
                    num_heads=num_heads,
                    dropout=dropout,
    ...
    def forward(self, x, data, t, att_extra_input=None):
        B, V, F = x.shape
        cond_x, edge_index, edge_attr = data.x, data.edge_index, data.edge_attr
        cond_x = cond_x.view(1, *cond_x.shape).expand(B, -1, -1)
        x_skip = x
        ...
        for i in range(len(self._gconv_layers) - 1):
            conv = self._gconv_layers[i]
            att_layer = self._attention_layers[i]
            lnorm = self._lnorm_layers[i]
            linear = self._linear_layers[i]
            att_embed_layer = self._att_extra_input_embed_layers[i]
            ...
            att_extra_embedded = att_embed_layer(x_att_features)
            x = x + att_extra_embedded

            x = att_layer(x)           # sliding window attention
        ...
        # final graph conv + attention
        x = self._gconv_layers[-1](x, edge_index, edge_attr=edge_attr) if self.use_edge_attr else self._gconv_layers[-1](x, edge_index)
        if self._cond_layer is not None:
            x = self._cond_layer(x, t)
        x = self._nonlinear(x)
        ...
        x = self._attention_layers[-1](x)     # sliding window attention
...
class SlidingWindowMultiHeadAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0.0, window_size=128, batch_first=True):
        super().__init__()
    ...
    def forward(self, x):
        # x: (B, V, C) if batch_first=True
        B, V, C = x.size()
        out_chunks = []
        num_chunks = (V + self.window_size - 1) 
        print(f"SlidingWindowMHA: window_size={self.window_size}, V={V}, num_chunks={num_chunks}")
        for start in range(0, V, self.window_size):
            end = min(start + self.window_size, V)
            x_chunk = x[:, start:end, :]                # (B, chunk_len, C)
            att_out, _ = self.mha(x_chunk, x_chunk, x_chunk)
            out_chunks.append(att_out)
        x_out = torch.cat(out_chunks, dim=1)            # (B, V, C)
        return x_out
...
class AttGNN(nn.Module):
    ...
    def forward(self, x, cond, t_embed):
        ...
        x_skip = x
        i = 0
        for block in self._gnn_blocks:  # self._gnn_blocks len = 14
            # nowMen = torch.cuda.memory_allocated() / 1024**3
            # print(f"{i} DEBUG: ATTGNN Allocated memory: {nowMen:.2f} GB Increase memory: {(nowMen-mem)*1024:.2f} MB")
            # mem = nowMen
            if isinstance(block, AttGNNBlock): # include attention conditioning
                att_input = x_skip if self.dir_att_input else x
                x = block(x, cond, t_embed, att_extra_input=att_input)   # 3 6 10 
            elif isinstance(block, MLP):
                x = block(x)                # 2 4 7 9 11 
            else:
                x = block(x, cond, t_embed)    # 0 1 5 8 12
            i += 1
        if self.use_enc:
            if self.in_node_features != self.out_node_features:
