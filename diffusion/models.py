...
class ContinuousDiffusionModel(nn.Module):
    ...
    def __init__(
            ...
            max_diffusion_steps = 1000, # default eval 
            ddim_max_diffusion_steps = 50,
        ...
        self.input_shape = input_shape
        self.max_diffusion_steps = max_diffusion_steps
        self.ddim_max_diffusion_steps = ddim_max_diffusion_steps # 50
    ...
    def __call__(self, x, cond, t):
        ...
        # print(f"DEBUG: __call__ Allocated memory: {torch.cuda.memory_allocated() / 1024**3:.2f} GB")
        return self._reverse_model(x, cond, t_embed).view(*x.shape)

    def loss(self, x, cond, _): # last input is for time index; ignored here 
        B = x.shape[0] # x is (B, V, F) for graphs   # batch size   V: 节点数量（graph中的module数）F: x y 坐标
        t = self._t_dist.sample((B,)).squeeze(dim = -1)   # [0, 1]
    ...
    def reverse_samples(self, B, x_in, cond, num_timesteps=-1, intermediate_every = 0, mask_override = None):
        ...
        if num_timesteps <= 0:
            ...
            # num_timesteps = self.ddim_max_diffusion_steps  # 50
            # num_timesteps = 30  ## !!!!!!!!! 这里只让推理30次
        # print(f"num_timesteps: {num_timesteps}")
        x = self._epsilon_dist.sample(batch_shape).squeeze(dim = -1) # (B, C, H, W)  初始噪声 x_T
        mask = mask_override.view(*mask_shape) if mask_override is not None else self.get_mask(x_in, cond)
        # assert mask is None or mask.sum() == 0, "masks not yet supported when sampling"
        x = torch.where(mask, x_in, x) if mask is not None else x  # 根据 mask 固定某些模块不被扩散影响 IO pin 不动

        intermediates = [x]  # 用于可视化中间布局演化
        ...
        # print(f"DEBUG: Memory before loop: {mem:.2f} MB")         
        ...
        for i, (t, t_minus_one) in enumerate(zip(timesteps[:-1], timesteps[1:])):
            ...
            # nowMen = torch.cuda.memory_allocated() / 1024**2
            # print(f"i: {i}, t: {t.item()}")
            # print(f"DEBUG: Allocated memory: {nowMen-mem:.2f} MB")
            # mem = nowMen
            if self.is_guided_sampling:
                if self.guidance_mode == "sgd":
                    # 在 sampling 时加入 RL guidance（比如降低 HPWL 或合法化 penalty）
                    guidance_fn = lambda x_hat: self.reverse_guidance_force(x_hat, cond, t, mask)
                elif self.guidance_mode == "opt":  # ibm_guided 进入的是这个分支
            ...
            # t_vec = torch.tensor(t, device=x.device).expand(B)
            t_vec = t.clone().detach().to(x.device).expand(B)
            eps_predict = self(x, cond, t_vec)  # 扩散模型预测噪声
        
            z = self._epsilon_dist.sample(batch_shape).squeeze(dim = -1) if i<(len(timesteps)-2) else torch.zeros_like(x)
            # 最后一次随机噪声用全0

            # 调度器执行一步反向扩散 对应公式 x_{t-1} = α_{t-1} * x0_predicted + σ_{t-1} ε + 额外噪声
            ...
            # # 测试只去噪一次的效果  # debug  !!!!!!!!!!!!!!!!!!!! 务必删去
            # return x0_predicted, intermediates

        return x, intermediates
    
    def reverse_samples_ddpo_old_1109(self, B, x_in, cond, num_timesteps=-1, intermediate_every=0, mask_override=None, output_log_prob=False):
        batch_shape = (B, cond.x.shape[0], self.input_shape[1])
        mask_shape = (1, x_in.shape[1], 1)

        if num_timesteps <= 0:
            # num_timesteps = self.max_diffusion_steps
            # V1.61 batch 4 = 30  batch 1 = 100 
            # v2.61 batch 2 = 30  batch 1 = 30
            num_timesteps = 25 #  
        
        x = self._epsilon_dist.sample(batch_shape).squeeze(dim = -1) # (B, C, H, W)   # 采样噪声样本 x_T
        mask = mask_override.view(*mask_shape) if mask_override is not None else self.get_mask(x_in, cond)
        # assert mask is None or mask.sum() == 0, "masks not yet supported when sampling"
        x = torch.where(mask, x_in, x) if mask is not None else x

        log_probs = []  # <--- 存储每步log_prob
        self._noise_scheduler.set_timesteps(num_timesteps)
        timesteps = self._noise_scheduler.timesteps
        ...
        # t = T, T - 1, ... , 1  从噪声逐步反向采样
        for i, (t, t_minus_one) in enumerate(zip(timesteps[:-1], timesteps[1:])):
            print(f"i: {i}, t: {t.item()}") # t 是 tensor, 用 .item()
            print(f"Allocated memory: {torch.cuda.memory_allocated() / 1024**3} GB")
            if self.is_guided_sampling:
                if self.guidance_mode == "sgd":
                    guidance_fn = lambda x_hat: self.reverse_guidance_force(x_hat, cond, t, mask)
            ...
            # t_vec = torch.tensor(t, device=x.device).expand(B)
            t_vec = t.clone().detach().to(x.device).expand(B)
            eps_predict = self(x, cond, t_vec)   # 模型在每一步的噪声预测
            # print(f"Output shape: {eps_predict.shape}")
            # print(f"Output memory usage: {eps_predict.element_size() * eps_predict.nelement() / 1024**2} MB")

            if output_log_prob:
                noise = self._epsilon_dist.sample(batch_shape).squeeze(dim=-1).detach()  # 新的噪音，存在问题
                dist = torch.distributions.Normal(eps_predict, torch.ones_like(eps_predict))
                step_log_prob = dist.log_prob(noise).mean(dim=(1, 2))  # (B)
                log_probs.append(step_log_prob)  # log_probs.append(step_log_prob.detach())
        
            z = self._epsilon_dist.sample(batch_shape).squeeze(dim = -1) if i<(len(timesteps)-2) else torch.zeros_like(x)
            x, _ = self._noise_scheduler.step(
                eps_prediction = eps_predict,
                t = t,
                t_minus_one = t_minus_one,
                xt = x,
        ...
        if output_log_prob:
            log_prob = torch.stack(log_probs, dim=0)  # (T, B)
            return x, log_prob
        else:
            return x

    # log_prob 计算图 显存占用大
    def reverse_samples_ddpo_old_1112(self, B, x_in, cond, num_timesteps=-1, intermediate_every=0, mask_override=None, output_log_prob=False):
        # x_in 的形状是 (B, C, H, W) 或 (B, feature_dim)
        batch_shape = x_in.shape # x_in 已经包含了正确的维度 (B, C, H, W)
        device = x_in.device

        if num_timesteps <= 0:
            # V1.61 batch 4 = 30 batch 2 = 60 batch 1 = 125 
            # v2.61 batch 2 = x  batch 1 = 25 
            num_timesteps = 60
        
        x = self._epsilon_dist.sample(batch_shape).squeeze(dim = -1).to(device) # (B, C, H, W)   # 采样噪声样本 x_T
        
        # 处理 mask，确保其在正确设备上
        mask = None
        if mask_override is not None:
            # 假设 mask_override 已经是正确的形状 (B, 1, H, W) 或 (B, features_dim)
            mask = mask_override.to(device)
        elif hasattr(self, 'get_mask') and x_in is not None and cond is not None:
             # 如果您有 get_mask 方法，并且 x_in 和 cond 都存在
            mask = self.get_mask(x_in, cond).to(device)
            
        if mask is not None:
            x = torch.where(mask, x_in, x) # 如果 mask 为真，则使用 x_in 的值


        # 核心改动 1：使用累积的对数概率，而非列表
        # 初始化为 B 个 0，因为每个批次样本都会有一个累积的 log_prob
        accumulated_log_prob = torch.zeros(B, device=device, dtype=x.dtype) 
        # log_probs = []
        ...
        for i, (t, t_minus_one) in enumerate(zip(timesteps[:-1], timesteps[1:])):
            ...
            print(f"i: {i}, t: {t.item()} node_size: {node_size}")
            print(f"DEBUG: Allocated memory: {nowMen:.2f} GB Increase memory: {nowMen-mem:.2f} GB Ratio(increase/node_size):{1000*(nowMen-mem)/(node_size)}")
            mem = nowMen
            ...
            # t_vec = torch.tensor(t, device=device).expand(B)
            # t_vec = t.clone().detach().to(device).expand(B)
            t_vec = t.to(device).expand(B) # 无需 clone().detach()
            # 模型预测噪声 eps_predict
            eps_predict = self(x, cond, t_vec)
            
            # 采样当前步的随机噪声 z，并将其传入 _noise_scheduler.step
            # 这里的 z 必须与 _noise_scheduler.step 内部使用的 z 是同一个
            z_current_step = self._epsilon_dist.sample(batch_shape).squeeze(dim = -1).to(device) \
                             if i < (len(timesteps) - 2) else torch.zeros_like(x).to(device) # timesteps[-1] 是最后一个 t_minus_one，不需要采样 z

            # 调用 _noise_scheduler.step，并获取 x_t_minus_one, mu_theta, sigma_s
            x_t_minus_one, mu_theta, sigma_s = self._noise_scheduler.step_ddpo(
                eps_prediction = eps_predict,
                t = t,
                t_minus_one = t_minus_one,
                xt = x,
                z = z_current_step, # 将 z 传入
                mask = mask,
                guidance_fn = guidance_fn,
                return_mean_and_std=True # 激活返回均值和标准差
            )
            
            if output_log_prob:
                # 策略 p_theta(xt-1 | xt, c) = N(xt-1 | mu_theta, sigma_s^2 I)
                current_dist = torch.distributions.Normal(mu_theta, sigma_s)
                
                # 计算实际生成的 x_t_minus_one 在这个分布下的对数概率
                # log_prob 在所有数据维度上求和，得到每个 batch 样本的对数概率 (B,)
                # 确保 x_t_minus_one 和 mu_theta, sigma_s 的形状一致
                step_log_prob = current_dist.log_prob(x_t_minus_one).sum(dim=tuple(range(1, x_t_minus_one.ndim))) # sum over all dimensions except batch

                # 核心改动 2：直接累加到 accumulated_log_prob
                accumulated_log_prob += step_log_prob
                # 确保 log_prob 保留梯度，因为它是用于策略梯度更新的
                # log_probs.append(step_log_prob)

            # del eps_predict, z_current_step, mu_theta, sigma_s, current_dist, step_log_prob
            x = torch.clamp(x_t_minus_one, -2, 2) # 更新 x 到下一步，并进行裁剪

        if output_log_prob:
            # log_prob = torch.stack(log_probs, dim=0)  # (T, B)
            return x, accumulated_log_prob
        else:
            return x

    # 新的方法只需要reward数值，不需要计算图
    @torch.no_grad()  # 关键：整个采样过程不需要梯度
    def reverse_samples_ddpo_old_1114(self, B, x_in, cond, num_timesteps=-1, intermediate_every=0, mask_override=None, output_log_prob=False):
        """
        优化版本：移除log_prob计算，使用torch.no_grad()降低显存
        """
        batch_shape = x_in.shape
        device = x_in.device

        if num_timesteps <= 0:
            num_timesteps = self.max_diffusion_steps  # 保持与原始一致
        
        # 采样初始噪声
        x = self._epsilon_dist.sample(batch_shape).squeeze(dim=-1).to(device)
        ...
        for i, (t, t_minus_one) in enumerate(zip(timesteps[:-1], timesteps[1:])):
            ...
            # 采样噪声
            z_current_step = self._epsilon_dist.sample(batch_shape).squeeze(dim=-1).to(device) \
                            if i < (len(timesteps) - 2) else torch.zeros_like(x).to(device)

            # 执行step（不需要梯度）
            x_t_minus_one, _ = self._noise_scheduler.step_ddpo(
                eps_prediction=eps_predict,
                t=t,
                t_minus_one=t_minus_one,
                xt=x,
                ...
                mask=mask,
                guidance_fn=guidance_fn,
                return_mean_and_std=False  # 不需要返回均值和标准差
            )
            
            # 更新x（裁剪防止数值不稳定）
            x = torch.clamp(x_t_minus_one, -2, 2)

        return x  # 只返回最终样本

    ######### 真的DDPO #########
    def reverse_samples_ddpo(self, B, x_in, cond, num_timesteps=-1, intermediate_every=0, 
                        mask_override=None, output_log_prob=False, return_all_steps=False):
        """
        修改版: 正确计算扩散过程的log_prob
        """
        batch_shape = x_in.shape
        device = x_in.device
        
        if num_timesteps <= 0:
            num_timesteps = 50  # 根据你的配置调整
        ...
        # 核心修正：存储每一步的真实噪声和预测噪声
        if output_log_prob:
            # 存储每一步的x_t, x_0, t, 预测噪声
            xt_history = []
            x0_history = []
            t_history = []
            pred_eps_history = []
        ...
        for i, (t, t_minus_one) in enumerate(zip(timesteps[:-1], timesteps[1:])):
            ...
            if output_log_prob:
                pred_eps_history.append(eps_predict.clone())
            ...
            if output_log_prob:
                ...
                x_t_minus_one, mu_theta, sigma_s = self._noise_scheduler.step_ddpo(
                    ...
                    z=z_current_step,
                    mask=mask,
                    guidance_fn=None,  # 在log_prob计算时不使用guidance
                    return_mean_and_std=True
                )
                
                # 保存x0预测用于噪声计算
                alpha_t = self._noise_scheduler.get_alpha_t(t_vec)
                sigma_t = self._noise_scheduler.get_sigma_t(t_vec)
                alpha_t = alpha_t.view(-1, 1, 1)
                sigma_t = sigma_t.view(-1, 1, 1)
                predicted_x0 = (x - sigma_t * eps_predict) / (alpha_t + 1e-8)
                x0_history.append(predicted_x0.clone())
            else:
                # 标准采样
                x_t_minus_one = self._noise_scheduler.step(
                    eps_prediction=eps_predict,
                    t=t,
                    t_minus_one=t_minus_one,
                    xt=x,
            ...
            if mask is not None:
                x_t_minus_one = torch.where(mask, x_in, x_t_minus_one)
        ...
        if output_log_prob:
            # 将历史记录转换为张量
            xt_history = torch.stack(xt_history, dim=0)  # (T, B, 316, 2)
            x0_history = torch.stack(x0_history, dim=0)  # (T, B, 316, 2)
            pred_eps_history = torch.stack(pred_eps_history, dim=0)  # (T, B, 316, 2)
            t_history = torch.stack(t_history, dim=0).to(device)  # (T,)
            
            # 计算每一步的真实噪声
            true_noises = []
            for i in range(len(t_history)):
                t_step = t_history[i].expand(B)
                xt_step = xt_history[i]
                x0_step = x0_history[i]
                true_noise = self.get_true_noise_from_xt(xt_step, x0_step, t_step)
                true_noises.append(true_noise)
            
            true_noises = torch.stack(true_noises, dim=0)  # (T, B, 316, 2)
            
            # 计算每一步的噪声预测MSE
            mse_per_step = torch.mean((pred_eps_history - true_noises) ** 2, dim=[2, 3])  # (T, B)
            
            # 获取每个时间步的方差
            _, sigma_history = self._noise_scheduler.get_alpha_sigma_batch(t_history)
            log_variances = torch.log(sigma_history ** 2 + 1e-8)  # (T,)
            log_variances = log_variances.view(-1, 1)  # (T, 1)
            
            # 计算每一步的对数概率（高斯分布）
            step_log_probs = -0.5 * (
                mse_per_step / (sigma_history ** 2 + 1e-8).view(-1, 1) + 
                log_variances + 
                torch.log(torch.tensor(2.) * torch.pi)
            )  # (T, B)
            
            # 重要：使用加权平均，不要对所有时间步求和
            T = step_log_probs.shape[0]
            # 早期时间步更重要，使用指数衰减权重
            weights = torch.exp(-0.1 * torch.arange(T, device=device, dtype=torch.float32)).view(-1, 1)
            weights = weights / weights.sum()  # 归一化
            
            # 加权平均得到轨迹log_prob
            trajectory_log_prob = torch.sum(step_log_probs * weights, dim=0)  # (B,)
            
            # 数值稳定性保护
            trajectory_log_prob = torch.clamp(trajectory_log_prob, -10.0, 10.0)
            
            # print(f"✅ Corrected trajectory_log_prob: {trajectory_log_prob}")
            return x, trajectory_log_prob
        
        return x

    def get_true_noise_from_xt(self, xt, x0, t):
        """
        从x_t和x_0计算真实噪声
        根据扩散过程：x_t = sqrt(alpha_t) * x_0 + sqrt(1-alpha_t) * noise
        """
        device = xt.device
        ...
        # 获取alpha_t
        # 获取alpha_t和sigma_t
        alpha_t = self._noise_scheduler.get_alpha_t(t)  # (B,)
        sigma_t = self._noise_scheduler.get_sigma_t(t)  # (B,)
        # 调整维度以便广播
        ...
        if x0.shape != xt.shape:
            # ChipDiffusion中x0应该和xt形状相同
            # 如果不匹配，可能需要reshape或插值
            print(f"⚠️ Shape mismatch: x0 {x0.shape} vs xt {xt.shape}")
            x0 = x0.view(xt.shape)
        
  
        # 核心公式: x_t = alpha_t * x_0 + sigma_t * noise
        # 因此: noise = (x_t - alpha_t * x_0) / sigma_t
        noise = (xt - alpha_t * x0) / (sigma_t + 1e-8) # 避免除零
    ...
    def _loss(self, prediction, target, mask = None):  # 求均方误差
        if mask is not None:
            numel = torch.numel(mask)
