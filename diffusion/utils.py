...
@torch.no_grad()
def validate_ddpo_hpwl_ratio(dataloader, model, num,
                             scale_factor=50.0,
                             legal_target=0.95,
                             penalty_C=None,
                             use_median=False,
    ...
    """
    改进的验证函数：
    - 返回丰富指标（mean/median/percentile, leg_mean, leg_rate, violation）
    - 不再把 illegal 直接置0，而是保留 r_hpwl 并同时计算带惩罚的 composite score
    Args:
      dataloader, model: 和你原来一致
      num: 采样次数（在 val pool 上抽 num 次）
      scale_factor: 把 r_hpwl 放大到可学习量级（你原来有）
      legal_target: 合法性门槛
      penalty_C: 如果 None 则设为 scale_factor * 2
      use_median: True 则对 r_hpwl 使用中位数统计（更稳健）
      device: torch device，若 None 则用 dataloader.device
    Returns:
      logs: dict 包含多种评估指标
    """
    if device is None:
        device = getattr(dataloader, "device", "cuda")

    model.eval()
    per_sample_r = []        # r_hpwl scaled, even for illegal keep value
    per_sample_leg = []      # legality scalar
    per_sample_penalized = []  # r with explicit penalty for illegal (for composite)

    if penalty_C is None:
        penalty_C = float(scale_factor) * 2.0
    ...
    with torch.no_grad():
        for _ in range(num):
            ...
            x = x.unsqueeze(0).to(device)       # (1, ...)
            cond = cond.to(device)

            # 生成（batch_size=1）
            x0, _ = model.reverse_samples(1, x[0], cond, intermediate_every=0)
            x0 = x0.detach()

            # 评估 legality & hpwl
            legality = check_legality_new(x0[0], x[0], cond, cond.is_ports, score=True)
            hpwl = hpwl_fast(x0[0], cond, normalized_hpwl=True)
            baseline_hpwl = hpwl_fast(x[0], cond, normalized_hpwl=True)

            # 计算原始相对改进（可能为负）
            # r_raw = 1 - hpwl / baseline_hpwl  # 原始小数，如 0.01 表示 1% 改进
            # scale 放大，便于 RL 学习
            if baseline_hpwl == 0:
                r_raw = 0.0
            else:
                r_raw = - (hpwl / baseline_hpwl)

            r_scaled = float(r_raw) * float(scale_factor)

            per_sample_r.append(r_scaled)
            per_sample_leg.append(float(legality))

            # 对 illegal 做惩罚化处理，便于计算 composite metrics
            if legality >= legal_target:
                per_sample_penalized.append(r_scaled)
            else:
                # 选项：将 illegal 的 score 设为 r_scaled - penalty_C
                per_sample_penalized.append(r_scaled - penalty_C)

    t1 = time.time()
    model.train()

    # 转 numpy 方便统计
    arr_r = np.array(per_sample_r, dtype=float)
    arr_leg = np.array(per_sample_leg, dtype=float)
    arr_pen = np.array(per_sample_penalized, dtype=float)

    # 统计项（多种：mean, median, percentiles, leg_rate）
    hpwl_mean_all = arr_r.mean()
    hpwl_median_all = np.median(arr_r)
    hpwl_25 = np.percentile(arr_r, 25)
    hpwl_75 = np.percentile(arr_r, 75)

    # 合法性统计：平均合法性、binary合法率（>=legal_target）
    leg_mean = arr_leg.mean()
    leg_rate = float((arr_leg >= legal_target).sum()) / max(1, len(arr_leg))
    violation = max(0.0, legal_target - leg_mean)  # 平均缺口

    # 统计仅对合法样本的 hpwl（如果没有合法样本，设为 NaN）
    legal_mask = (arr_leg >= legal_target)
    if legal_mask.any():
        arr_r_valid = arr_r[legal_mask]
        valid_mean = float(arr_r_valid.mean())
        valid_median = float(np.median(arr_r_valid))
    else:
        valid_mean = float("nan")
        valid_median = float("nan")

    # composite score（软惩罚），以 arr_pen 的 mean 作为 score
    composite_mean = float(arr_pen.mean())

    logs = {
        "hpwl_mean_all": float(hpwl_mean_all),
        "hpwl_median_all": float(hpwl_median_all),
        "hpwl_25": float(hpwl_25),
        "hpwl_75": float(hpwl_75),
        "hpwl_mean_valid": valid_mean,
        "hpwl_median_valid": valid_median,
        "leg_mean": float(leg_mean),
        "leg_rate": float(leg_rate),
        "violation": float(violation),
        "composite_mean_penalized": composite_mean,
        "num_samples": int(len(arr_r)),
        "eval_time": float(t1 - t0)
    }
    print(f" [DEBUG] Validate: hpwl_mean_valid:{valid_mean} leg_mean:{float(leg_mean)} eval_time: {float(t1 - t0)}")

    return logs

@torch.no_grad()
def validate_ddpo_hpwl_ratio_1212(dataloader, model, num, scale_factor, legal_target):
    model.eval()
    rewards = []
    ...
    for _ in range(num):
        ...
        # 生成 sample 验证的时候 batch_size = 1
        x0, _ = model.reverse_samples(
        ...
        baseline_hpwl = hpwl_fast(x[0], cond, normalized_hpwl=True)
        if legality >= legal_target:
            rewards.append((1 - hpwl/baseline_hpwl)* scale_factor)
        else:
            rewards.append(0)

    logs = {
        "hpwl_mean": (sum(rewards) / len(rewards))
    }
    t_total_end = time.time()
    # print("============== Validation Time Summary ==============")
    # print(f"\nhpwl_mean:{sum(rewards) / len(rewards)} Validate Total time: {t_total_end - t_total_start:.2f} sec")
    # print("======================================================")
    model.train()
    return logs
...
@torch.no_grad()
def validate_ddpo_hpwl(dataloader, model, num = 40):
    model.eval()
    ...
    # 遍历固定验证子集
    i = 0
    for idx in dataloader.val_indices:
        if i > num:
            break
        i += 1
        # t_item_start = time.time()
        x, cond = dataloader.val_set[int(idx)]
        x = x.unsqueeze(0).to(dataloader.device)       # (1, ...)
        # x = x.to(dataloader.device).view(1, *x.shape).expand(batch_size, *x.shape)
        ...
        # ddpo_model.reward_fn(samples[0], cond)  # 近似值
        # 真实 HPWL
        hpwl_normalized, hpwl_rescaled = hpwl_fast(
            samples[0], cond, normalized_hpwl=False
        )
        hpwls.append(hpwl_normalized)
        # t_item_end = time.time()
        # print(f"[VAL] idx={int(idx)}, time={t_item_end - t_item_start:.4f} sec")
    
    t_total_end = time.time()
    hpwls = torch.tensor(hpwls, dtype=torch.float32)
    hpwls_mean = hpwls.mean().item()
    # print("============== Validation Time Summary ==============")
    # print(f"\nhpwl_mean:{hpwls_mean} Validate Total time: {t_total_end - t_total_start:.2f} sec")
    # print("======================================================")

    logs = {
        "hpwl_mean": hpwls_mean,
        # "hpwl_min": hpwls.min().item(),
        # "hpwl_max": hpwls.max().item(),
        # "hpwl_std": hpwls.std().item()
    }
    model.train()
    return logs
...
def validate_ddpo(dataloader, model, ddpo_model, hpwl_w, legality_w, val_size = 16):
    ...
    # print("============== Validation Time Summary ==============")
    print(f" [DEBUG] Validate: reward:{rewards.mean().item():.3f} | eval_time: {float(t_total_end - t_total_start):.3f}")
    # print("======================================================")
...
def save_outputs(
    ...
    # 评测时间 忽略画图
    if drawPic:
        image0 = visualize_placement(x_in[0], cond, plot_pins=True, plot_edges=False, img_size=(2048, 2048))
        debug_plot_img(image0, os.path.join(save_folder, f"{idx}_origin"), outputName = False)
    ...
    # save orgin image too
    

    t1 = time.time()
    # print(f"pre time:{t1-t0}")
    if cond_preprocessed.num_nodes == 0:
        # handle edge case with 0 nodes after preprocessing
        sample = torch.zeros_like(x_preprocessed)
    else:
        if policy == "open_loop":  # sample就是生成的布局
            sample, intermediates, policy_metrics_special = policies.open_loop(1, model, x_preprocessed, cond_preprocessed, intermediate_every = 0, save_videos = policy_kwargs["save_videos"])
            metrics_special.update(policy_metrics_special)
            # 画中间图
            if policy_kwargs["save_videos"]:
                for temp, x_i in enumerate(intermediates):
                    original_hpwl_t = hpwl_fast(x_i[0], cond_preprocessed, normalized_hpwl=True)
                    print(f"{temp}: hpwl: {original_hpwl_t}")
                    image_temp = visualize_placement(x_i[0], cond, plot_pins=True, plot_edges=False, img_size=(2048, 2048))
                    path_temp = os.path.join(save_folder, f"{idx}_intermediates")
                    os.makedirs(path_temp, exist_ok=True)
                    path_temp = os.path.join(path_temp, f"intermediates_{temp}")
                    debug_plot_img(image_temp, path_temp, outputName = False)
    ...
    if drawPic:
        debug_plot_img(image_legalized, os.path.join(save_folder, f"{idx}_placed"))
    # t21 = time.time()
    # print(f"legal time:{t21-t2}")
    ...
    metrics.update({
        "idx": idx,
        "hpwl_normalized": hpwl_normalized,  # 生成的布局线长
        ...
        "original_hpwl_normalized": original_hpwl_normalized, # 输入时的线长
        "hpwl_ratio": hpwl_normalized/max(1e-12, original_hpwl_normalized),  # 优化比率
        "model_time": t2-t1,
...
class GraphDataLoader:
    def __init__(
        ...
        self.device = train_device
        self.train_batch_size = train_batch_size  # 就是cfg.batch_size
        self.val_batch_size = val_batch_size
        self.train_set = train_dataset
        self.val_set = val_dataset
        ...
        # 用于DDPO验证时候的数据集数量，避免每次随机。 取512条
        # self.val_indices = torch.randperm(len(self.val_set))[:min(512, len(self.val_set))]


         # -------------------------- 核心新增：按节点数分组 --------------------------
        # self.train_groups = self._group_by_node_num()  # 训练集分组结果
        # self.val_groups = self._group_by_node_num(is_val=True)  # 验证集分组结果

    def _group_by_node_num(self, is_val=False):
        """按网表节点数分组: key=节点数: value=该节点数的样本列表（(x, y))"""
        groups = defaultdict(list)
        dataset = self.val_set if is_val else self.train_set
        for idx in range(len(dataset)):
            x, y = dataset[idx]
            node_num = x.shape[0]  # 假设x的形状是(node_num, feature_dim)，取节点数
            groups[node_num].append( (x, y) )  # 存储该节点数的样本

        # -------------------------- 新增：打印分组统计 --------------------------
        dataset_name = "验证集" if is_val else "训练集"
        print(f"\n{dataset_name}分组统计：")
        print(f"共包含 {len(groups)} 种不同的节点数")
        total_samples = 0
        # 遍历groups，打印每个节点数对应的样本数
        for node_num, samples in sorted(groups.items()):  # sorted按节点数升序排列
            sample_count = len(samples)
            total_samples += sample_count
            print(f"  节点数 {node_num}：{sample_count} 个样本")
        print(f"{dataset_name}总样本数：{total_samples}")
        # ------------------------------------------------------------------------
    ...
    def get_batch_with_idx(self, split, idx):
        assert split in ("train", "val"), "split argument has to be one of 'train' or 'val'"
        dataset = self.train_set if split=="train" else self.val_set
        batch_size = self.train_batch_size if split=="train" else self.val_batch_size
    ...
    def get_batch_and_idx(self, split):
        assert split in ("train", "val"), "split argument has to be one of 'train' or 'val'"
        dataset = self.train_set if split=="train" else self.val_set
        batch_size = self.train_batch_size if split=="train" else self.val_batch_size
    ...
    # 同时获取多个case，但是cond的处理，输入模型还是存在疑问
    def get_muti_batch(self, split):
        assert split in ("train", "val"), "split must be 'train' or 'val'"
        batch_size = self.train_batch_size if split == "train" else self.val_batch_size
        groups = self.train_groups if split == "train" else self.val_groups

        if split == "train":
            selected_node_num = random.choice(list(groups.keys()))
            selected_samples = groups[selected_node_num]
            origin_sample_count = len(selected_samples)
            
            # 动态补全样本（逻辑不变）
            if origin_sample_count < batch_size:
                completed_samples = []
                for i in range(batch_size):
                    completed_samples.append( selected_samples[i % origin_sample_count] )
            else:
                completed_samples = selected_samples
            
            # 采样（逻辑不变）
            if self.is_shuffle[split]:
                sampled_pairs = random.sample(completed_samples, k=batch_size)
            else:
                start_idx = self.current_idx[split]
                end_idx = start_idx + batch_size
                if end_idx > len(completed_samples):
                    sampled_pairs = completed_samples[start_idx:] + completed_samples[:end_idx - len(completed_samples)]
                    self.current_idx[split] = end_idx - len(completed_samples)
                else:
                    sampled_pairs = completed_samples[start_idx:end_idx]
                    self.current_idx[split] = end_idx
            
            # -------------------------- 关键修改：x和y的拼接逻辑 --------------------------
            # 1. x的拼接：不变（张量可直接stack）
            x_list = [pair[0] for pair in sampled_pairs]  # 每个x：(node_num, 2)
            x_batch = torch.stack(x_list, dim=0).to(self.device)  # (batch_size, node_num, 2)
            
            # 2. y的拼接：用Batch.from_data_list（适配Data对象）
            y_list = [pair[1] for pair in sampled_pairs]  # 每个y：Data对象
            y_batch = Batch.from_data_list(y_list).to(self.device)  # 拼接成Batch对象
            # ------------------------------------------------------------------------

        else:
            if self.val_batch_size == 1:
                idx = torch.randint(0, len(self.val_set), [1]) if self.is_shuffle[split] else self.current_idx[split]
                if not self.is_shuffle[split]:
                    self.current_idx[split] = (self.current_idx[split] + 1) % len(self.val_set)
                x, y = self.val_set[idx]
                x_batch = x.to(self.device).view(1, *x.shape)  # (1, node_num, 2)
                y_batch = y.to(self.device)  # 单样本无需拼接，直接用Data对象
            else:
                selected_node_num = random.choice(list(groups.keys()))
                selected_samples = groups[selected_node_num]
                origin_sample_count = len(selected_samples)
                ...
                # 动态补全
                if origin_sample_count < self.val_batch_size:
                    completed_samples = []
                ...
                sampled_pairs = random.sample(completed_samples, k=self.val_batch_size)
                # x拼接：不变
                x_batch = torch.stack([p[0] for p in sampled_pairs], dim=0).to(self.device)
                # y拼接：用Batch.from_data_list
                y_batch = Batch.from_data_list([p[1] for p in sampled_pairs]).to(self.device)

        # 调用prepare_output（注意：y_batch现在是Batch对象，需适配）
        output = self.prepare_output(x_batch, y_batch)
        return output
...
def visualize_placement(x, cond, plot_pins = False, plot_edges = False, img_size = (256, 256), mask = None):
    ...
    inbounds = torch.logical_and(left_bottom >= -1, right_top <= 1)    # -1 到 1为有效区间
    ...
    # cropped_image = base_image.crop(base_image.getbbox())  # 裁剪掉白边
...
def debug_plot_img(x, name = "debug_img", rescale = False, autoscale = False, outputName = True):
    ...
    plt.figure()
    plt.imshow(x) 
    plt.axis('off')  # Disable axis and ticks
    # plt.savefig(name, dpi=1000)
    plt.savefig(name, dpi=1000, bbox_inches='tight', pad_inches=0)
    plt.close()
    if outputName:
        print(f"fig save in {name}.png")
...
def calcul_baseline_hpwl(dataloader, model):
    ...
    for idx in range(size):
        ...
        samples, _ = model.reverse_samples(1, x, cond, intermediate_every = 0)
