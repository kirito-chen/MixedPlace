...
def cluster(input_cond, num_clusters, ubfactor=5, temp_dir = "logs/temp", verbose = False, placements = None, algorithm = "hmetis"):
    """
...
def main():
    ...
    for i in range(100, 2501, 100):  # 7501
        ...
        for j in range(len(data)):
            x = data[j][0]
            cond = data[j][1]
            # generate生成的时候的bug 这里修复。 原因是生成的时候多加了cell的数量
            N = cond.x.shape[0]
            cond.is_macros = cond.is_macros[:N]
            cond.is_ports = cond.is_ports[:N]
            num_clusterable = (~(cond.is_macros | cond.is_ports)).sum()
            sample = cluster(cond, min(512,num_clusterable), temp_dir = "logs/temp", placements = x.unsqueeze(dim=0), algorithm = "hmetis")
